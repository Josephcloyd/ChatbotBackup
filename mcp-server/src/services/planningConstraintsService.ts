...
function inferDefaultWeekdaysOnly(description: string): boolean {
  return /\b(?:office|company|team|employee|employees|staff|people|annotators?|workers?|agents?|members?|onboarding|encoding|encode|records?|validation|production|work|software|developers?|manufacturing|machines?|documents?|receipts?)\b/i.test(
    description,
  );
}
...
function extractDeliverables(description: string): string[] {
  const requested = [
    ...
    [
      ...
      /\bweekly\s+(?:progress\s+)?tracking|weekly\s+summary|weekly\s+report\b/i,
    ],
...
function extractWorkingDays(description: string): number[] | undefined {
  ...
  if (
    !/\b(?:on|only|every)\s+(?:mondays?|tuesdays?|wednesdays?|thursdays?|fridays?|saturdays?|sundays?)\b/i.test(
      description,
    ) &&
...
function normalizeQuantity(
  ...
  const multiplier =
    normalizedSuffix === "k" ? 1_000 : normalizedSuffix === "m" ? 1_000_000 : 1;
...
export function extractRequestedConstraints(
  ...
  const durationMatch =
    description.match(
      new RegExp(
        String.raw`\b(?:within|for|over|during)?\s*${numberToken}\s*[- ]?(?:calendar\s+)?(days?|weeks?|months?)\b`,
        "i",
      ),
    ) ??
    description.match(
      new RegExp(
        String.raw`\bnext\s+${numberToken}\s+(weekdays?|business\s+days?)\b`,
        "i",
      ),
    );
  const weekdayDurationMatch =
    description.match(
      new RegExp(
        String.raw`\b(?:within|for|over|during)?\s*${numberToken}\s+(weekdays?|business\s+days?)\b`,
        "i",
      ),
    ) ??
    description.match(
      new RegExp(
        String.raw`\b(?:for\s+)?(?:the\s+)?next\s+${numberToken}\s+(weekdays?|business\s+days?)\b`,
        "i",
      ),
    );
  const hoursMatch = description.match(
    new RegExp(
      String.raw`\b${numberToken}\s+(?:total\s+|working\s+|productive\s+)?hours?\b`,
      "i",
    ),
  );
  const videoDurationTargetMatch =
    description.match(
      /\b(?:total\s+of\s+|target\s+(?:of\s+|is\s+)?|deliver\s+|produce\s+(?:a\s+)?total\s+of\s+)?([\d,]+)(?:\s*([kKmM]))?\s*[- ]?hours?\s+(?:of\s+)?(?:[^.!?\n]{0,90}?\b)?(?:video[-\s]+)?recordings?\b/i,
    ) ??
    description.match(
      /\b([\d,]+)(?:\s*([kKmM]))?\s*[- ]?hours?\s+(?:refers?\s+to|means|is)\b[^.!?\n]{0,160}\b(?:completed\s+)?video\s+duration\b/i,
    ) ??
    description.match(
      /\b([\d,]+)(?:\s*([kKmM]))?\s*[- ]?hour\s+target\b[^.!?\n]{0,160}\b(?:duration|recordings?|approved|usable|quality\s+review)\b/i,
    );
  const teamMatch =
    description.match(
      new RegExp(
        String.raw`\b(?:class|team|group|crew)\s+of\s+${numberToken}\b`,
        "i",
      ),
    ) ??
    description.match(
      new RegExp(
        String.raw`\b${numberToken}\s+(?:active\s+)?(?:video\s+|audio\s+|field\s+|production\s+)?(?:annotators?|workers?|agents?|people|members?|employees?|staff|encoders?|resources?|developers?|engineers?|testers?|reviewers?|designers?|writers?|editors?|machines?|operators?|recorders?)\b`,
        "i",
      ),
    );
  const normalizedDates = extractNormalizedDateConstraints(
    description,
    currentDate,
  );
  ...
  if (videoDurationTargetMatch) {
    constraints.totalQuantity = normalizeQuantity(
      videoDurationTargetMatch[1]!,
      videoDurationTargetMatch[2],
    );
    constraints.unitOfMeasure = "video hours";
  }
  if (hoursMatch && !videoDurationTargetMatch) {
    const tail = description.slice(
      (hoursMatch.index ?? 0) + hoursMatch[0].length,
      (hoursMatch.index ?? 0) + hoursMatch[0].length + 24,
    );
    if (
      !/^\s*(?:per|\/)\s*(?:weekday|day|worker|resource|person|machine|shift)/i.test(
        tail,
      )
    ) {
  ...
  if (
    /\b(?:weekdays?\s+only|business\s+days?|monday\s+(?:through|to|-)\s+friday|avoid\s+weekends?|excluding\s+weekends?|no\s+weekends?)\b/i.test(
      description,
    )
  ) {
    constraints.weekdaysOnly = true;
  } else if (
    /\b(?:calendar\s+(?:days?|weeks?|months?)|including\s+weekends?|weekends?\s+included|seven\s+days\s+a\s+week)\b/i.test(
      description,
    )
  ) {
  ...
  if (normalizedDates.startDate) {
    ...
    if (normalizedDates.startDate < currentDate)
      constraints.allowPastDates = true;
  } else if (
    /\b(?:starting|starts?|from)(?:\s+date\s+of)?\s+today\b/i.test(description)
  ) {
  ...
  if (/\bhigh\s+priority|urgent|rush\b/i.test(description))
    constraints.priority = "high";
  else if (/\blow\s+priority\b/i.test(description))
  ...
  if (/\bqa|quality\s+assurance|quality\s+checks?\b/i.test(description))
    constraints.needsQa = true;
  if (/\breview|sign-?off|approval\b/i.test(description))
    constraints.needsReview = true;
  if (/\bbuffer|contingency\b/i.test(description))
    constraints.needsBuffer = true;
  if (
    /\bweekly\s+(?:progress\s+)?tracking|weekly\s+summary|weekly\s+report\b/i.test(
      description,
    )
  ) {
  ...
  const p1 = new RegExp(`\\b${NUM_PAT}\\s+(${UNIT_PAT})\\b`, "i").exec(
    description,
  );
  if (constraints.totalQuantity) {
    ...
    if (p2) {
      ...
      const ambiguousVideoCrewCount =
        p3 &&
        /^videos?$/i.test(p3[1]!) &&
        /\b(?:recorders?|operators?|staff|workers?|team)\b/i.test(
  ...
  if (!constraints.totalQuantity) {
    const p4 = new RegExp(
      String.raw`\btarget\s+(?:number\s+of\s+)?(?:(${UNIT_PAT})\s*(?:engaged|collected|processed|done|annotated)?\s+is\s+)?${NUM_PAT}\b`,
      "i",
    ).exec(description);
    if (p4 && p4[2]) {
  ...
  const tpHour = new RegExp(
    `\\b(\\d+(?:\\.\\d+)?)\\s+${UNIT_PAT}\\s*(?:\\/|per)\\s*(?:person\\s*(?:\\/|per)\\s*)?hour`,
    "i",
  ).exec(description);
  if (tpHour) constraints.throughputRate = Number(tpHour[1]);

  // "400 images per day", "400 images/day", "400 images per person per day"
  const tpDay = new RegExp(
    `\\b(\\d+(?:\\.\\d+)?)\\s+${UNIT_PAT}\\s*(?:\\/|per)\\s*(?:person\\s*(?:\\/|per)\\s*)?day`,
    "i",
  ).exec(description);
...
export function resolvePlanningSettings(
  ...
  const weekdaysOnly =
    requested.weekdaysOnly ??
    defaults.weekdaysOnly ??
    inferDefaultWeekdaysOnly(description);
  const workingDays = requested.workingDays ?? defaults.workingDays;
  const defaultStartDate = weekdaysOnly
  ...
  const startsToday = /\b(?:starting|starts?|from)\s+today\b/i.test(
    description,
  );
  const startDate = startsToday
    ? currentDate
    : (requested.startDate ?? defaultStartDate);
  ...
  if (!requested.allowPastDates && startDate < currentDate) {
    throw new Error(
      `Planning start date ${startDate} is earlier than ${currentDate}`,
    );
  }
  const endDate = requested.endDate;
  if (
    endDate !== undefined &&
    (!/^\d{4}-\d{2}-\d{2}$/.test(endDate) || !toIsoDate(endDate))
  ) {
    throw new Error("Planning end date must use YYYY-MM-DD format");
  }
  if (endDate !== undefined && endDate < startDate) {
    throw new Error(
      `Planning end date ${endDate} is earlier than start date ${startDate}`,
    );
  }
  if (
  ...
  let totalHours = positive(
    requested.totalHours,
    positive(defaults.totalHours, 160, "totalHours"),
    "totalHours",
  );
  if (
    ...
    const scheduledDays = buildScheduleDates({
      ...
      teamSize: Math.round(
        positive(
          requested.teamSize,
          positive(defaults.teamSize, 1, "teamSize"),
          "teamSize",
        ),
      ),
      weekdaysOnly,
...
export function buildScheduleDates(
  ...
  const isScheduledDay = (date: Date): boolean => {
    if (settings.workingDays?.length)
      return settings.workingDays.includes(date.getUTCDay());
  ...
  if (settings.endDate) {
    ...
    if (dates.length === 0)
      throw new Error("The requested period contains no scheduled workdays");
    return dates;
  }
