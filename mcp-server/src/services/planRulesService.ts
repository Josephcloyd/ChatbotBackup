...
function numericValue(
  ...
  if (!Number.isFinite(value)) {
    throw new Error(
      `Production Plan row ${rowNumber} must contain a numeric ${column}`,
    );
  }
  return value;
}
...
function targetColumn(columns: string[]): string {
  return (
    ...
    columns.find((column) => /target\s+(?:total\s+)?hours/i.test(column)) ??
    columns[2] ??
    "Target Total Hours"
  );
}
...
function teamColumn(columns: string[]): string {
  return (
    columns.find((column) =>
      /target\s+active\s+(?:annotators|recorders|operators|resources)|active\s+(?:annotators|recorders|operators|resources)|workers|team|staff|resource/i.test(
        column,
      ),
...
function allocateWeightedInteger(
  total: number,
  counts: number[],
...
function distributeLpbInteger(total: number, totalRows: number): number[] {
  ...
  const phaseTargets = allocateWeightedInteger(
    Math.round(total),
    counts,
    LPB_WEIGHTS,
  );
...
function distributeLpbHours(totalHours: number, totalRows: number): number[] {
  return distributeLpbInteger(Math.round(totalHours * 100), totalRows).map(
    (value) => value / 100,
  );
}
...
export class PlanRulesService {
  validate(plan: ProductionPlan, options: PlanRuleOptions): ProductionPlan {
    const sheet = plan.workbook.sheets.find(
      (item) => item.sheetName === "Production Plan",
    );
    ...
    if (sheet.rows.length === 0) {
      console.warn(
        "[planRulesService] Production Plan sheet had 0 rows. Synthesizing schedule rows from project constraints.",
      );
      const settings = resolvePlanningSettings(
        options.input.projectDescription,
        options.currentDate,
      );
      const constraints = extractRequestedConstraints(
        options.input.projectDescription,
        options.currentDate,
      );
      ...
      const hourTargets = useLpbModel
        ...
        : distributeInteger(
            Math.round(settings.totalHours * 100),
      ...
      scheduleDates.forEach((dateStr, rowIndex) => {
        ...
        sheet.columns.forEach((col) => {
          ...
          if (col === dateColumn) rowObj[col] = dateStr;
          ...
          } else if (semantic === "planned_staff")
            rowObj[col] = settings.teamSize;
          else if (semantic === "planned_output_per_person") {
            rowObj[col] = Number(
              (target / Math.max(settings.teamSize, 1)).toFixed(2),
            );
          } else if (semantic === "planned_hours")
            rowObj[col] = hourTargets[rowIndex] ?? 0;
            ...
            rowObj[col] = lpbNote(
              phase,
              rowIndex - phaseStart,
              lpbCounts[phaseIndex] ?? 0,
            );
          } else if (useLpbModel && /expected\s+completion/i.test(col)) {
            rowObj[col] =
              totalUnits > 0
                ? Number(((accum / totalUnits) * 100).toFixed(2))
                : "";
          } else if (
            /accumulate|accumulative/i.test(col) &&
            /target|plan/i.test(col)
          )
            rowObj[col] = accum;
          else rowObj[col] = "";
        });
    ...
    sheet.rows.forEach((row, index) => {
      ...
      if (!date) {
        throw new Error(
          `Production Plan row ${rowNumber} Date must use YYYY-MM-DD format`,
        );
      }
      const constraints = extractRequestedConstraints(
        options.input.projectDescription,
      ...
      if (constraints.weekdaysOnly === true && !isWeekday(parseIsoDate(date))) {
        throw new Error(
          `Production Plan row ${rowNumber} schedules weekend date ${date}`,
        );
      }
      if (
        constraints.workingDays?.length &&
        !constraints.workingDays.includes(parseIsoDate(date).getUTCDay())
      ) {
        throw new Error(
          `Production Plan row ${rowNumber} is outside the requested custom working days`,
        );
      }
      if (seenDates.has(date))
        throw new Error(`Production Plan contains duplicate date ${date}`);
      seenDates.add(date);
    ...
    if (constraints.duration !== undefined) {
      ...
      if (!scheduleMatches && expectedDates.length > 0) {
        console.warn(
          `[planRulesService] Auto-expanding ${actualDates.length} sample rows to complete ${expectedDates.length}-day schedule.`,
        );
        const useLpbModel = isLpbModel(constraints.planningModel);
        const totalUnits =
          settings.totalQuantity ??
          (settings.totalHours !== 160 ? settings.totalHours : undefined) ??
          100;
        const targets = useLpbModel
        ...
        sheet.rows = expectedDates.map((dateStr, rowIndex) => {
          ...
          sheet.columns.forEach((col) => {
            ...
            if (col === dateColumn) rowObj[col] = dateStr;
              ...
              rowObj[col] = settings.teamSize;
            else if (semantic === "role_headcount")
              rowObj[col] = firstExistingRow[col] ?? 0;
            else if (semantic === "planned_output_per_person") {
              rowObj[col] = Number(
                (target / Math.max(settings.teamSize, 1)).toFixed(2),
              );
      ...
      constraints.durationDays !== undefined &&
      sheet.rows.length !== constraints.durationDays
    ) {
      throw new Error(
    ...
    if (constraints.teamSize !== undefined) {
      ...
      if (teamValues.some((value) => value > constraints.teamSize!)) {
        throw new Error(
          `Production Plan exceeds requested team size of ${constraints.teamSize}`,
        );
      }
      if (!teamValues.some((value) => value === constraints.teamSize)) {
        throw new Error(
          `Production Plan never schedules the requested team size of ${constraints.teamSize}`,
        );
      }
    }
    if (
    ...
    const sheetNames = new Set(
      plan.workbook.sheets.map((item) => item.sheetName),
    );
    ...
    if (taskSheet) {
      ...
      for (const [index, row] of taskSheet.rows.entries()) {
        const taskId =
          typeof row["Task ID"] === "string" ? row["Task ID"].trim() : "";
        if (!taskId)
          throw new Error(`Task Breakdown row ${index + 1} is missing Task ID`);
        if (taskIds.has(taskId))
          throw new Error(
            `Task Breakdown contains duplicate Task ID ${taskId}`,
          );
        taskIds.add(taskId);
      }
      ...
      for (const [index, row] of taskSheet.rows.entries()) {
        ...
        if (!start || !end) {
          throw new Error(
            `Task Breakdown row ${rowNumber} must use YYYY-MM-DD planned start/end dates`,
          );
        }
        if (end < start)
          throw new Error(
            `Task Breakdown row ${rowNumber} ends before it starts`,
          );
        if (
          start < String(plan.project.startDate) ||
          end > String(plan.project.deadline)
        ) {
          throw new Error(
            `Task Breakdown row ${rowNumber} is outside the project period`,
          );
        }
        if (
          constraints.weekdaysOnly === true &&
          (!isWeekday(parseIsoDate(start)) || !isWeekday(parseIsoDate(end)))
        ) {
          throw new Error(
            `Task Breakdown row ${rowNumber} schedules weekend work in a weekdays-only plan`,
          );
        }
        if (
          constraints.workingDays?.length &&
          ...
          throw new Error(
            `Task Breakdown row ${rowNumber} is outside the requested custom working days`,
          );
        }
        for (const dependencyId of splitIds(row.Dependencies)) {
          if (!taskIds.has(dependencyId)) {
            throw new Error(
              `Task Breakdown row ${rowNumber} references unknown dependency ${dependencyId}`,
            );
          }
        }
      }
    ...
    const chartSheet = plan.workbook.sheets.find(
      (item) => item.sheetName === "Chart Specs",
    );
    if (chartSheet) {
      chartSheet.rows.forEach((row, index) => {
        const source =
          typeof row["Source Worksheet"] === "string"
            ? row["Source Worksheet"].trim()
            : "";
        if (!source || !sheetNames.has(source)) {
          throw new Error(
            `Chart Specs row ${index + 1} references unknown source worksheet "${source}"`,
          );
        }
      });
    }
