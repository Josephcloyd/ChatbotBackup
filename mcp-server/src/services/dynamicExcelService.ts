...
function writeGenericSheet(
  ...
  sheetDefinition.rows.forEach((row) => {
    sheet.addRow(
      sheetDefinition.columns.map((column) => safeCellValue(row[column] ?? "")),
    );
  });
...
export class DynamicExcelService {
  ...
  async writeDynamicProductionPlan(
    ...
    if (!productionSheet)
      throw new Error("Dynamic plan is missing the Production Plan sheet");
    ...
    const requiredColumn = (
      ...
      if (!column)
        throw new Error(`Dynamic plan is missing required ${semantic} column`);
      return column;
    };
    const columnNumber = (semantic: ProductionPlanColumnSemantic): number => {
      const required = requiredColumn(semantic);
      return (
        columnDefinitions.findIndex(
          (column) => column.label === required.label,
        ) + 1
      );
    };
    const columnLetter = (semantic: ProductionPlanColumnSemantic): string =>
      workbook
        .getWorksheet("Production Plan")
        ?.getColumn(columnNumber(semantic)).letter ??
    ...
    summary.getRow(8).values = [
      "Start Date",
      new Date(`${plan.project.startDate}T00:00:00Z`),
      "Deadline",
      new Date(`${plan.project.deadline}T00:00:00Z`),
    ];
    summary.getRow(9).values = [
      "Schedule",
      settings.weekdaysOnly ? "Weekdays only" : "Calendar days",
      "Scheduled Days",
      planRows.length,
    ];
    ...
    const metricLabel = result.unitLabel
      ? result.unitLabel.toUpperCase()
      : "HOURS";
    ...
    const plannedResult =
      result.unitLabel != null && result.totalQuantity != null
    ...
    summary.getCell("A12").value = {
      formula: `SUM('Production Plan'!${targetLetter}2:${targetLetter}${lastPlanRow})`,
      result: plannedResult,
    };
    summary.getCell("C12").value = {
      formula: `MAX('Production Plan'!${staffLetter}2:${staffLetter}${lastPlanRow})`,
      result: settings.teamSize,
    };
    summary.getCell("E12").value = {
      formula: `SUM('Production Plan'!${actualLetter}2:${actualLetter}${lastPlanRow})`,
      result: 0,
    };
    summary.getCell("G12").value = {
      formula: `IF(COUNT('Production Plan'!${actualLetter}2:${actualLetter}${lastPlanRow})=0,"",IFERROR(E12/A12,""))`,
      result: "",
    };
      ...
      const rowValues = columnDefinitions.map((column) => {
        ...
        if (column.semantic === "planned_output_per_person") {
          return {
            ...
            result: Number(
              (targetValue / Math.max(teamSize, 1)).toFixed(
                isQuantityMode ? 0 : 2,
              ),
            ),
          };
    ...
    for (let row = 2; row <= lastPlanRow; row += 1) {
      production.getCell(row, actualStaffColumnNumber).dataValidation = {
        type: "whole",
        operator: "between",
        formulae: [0, settings.teamSize],
        allowBlank: true,
        showErrorMessage: true,
        errorTitle: "Invalid team size",
        error: `Enter a value from 0 to ${settings.teamSize}.`,
      };
      production.getCell(row, actualOutputColumnNumber).dataValidation = {
        ...
        error: `${primaryActualColumn.label} cannot be negative.`,
      };
    }
    ...
    monthly.addRow([
      "Month",
      `Planned ${monthlyMetricLabel}`,
      `Actual ${monthlyMetricLabel}`,
      "Variance",
      "Completion Rate",
    ]);
    ...
    planRows.forEach((row) => {
      ...
      monthTargets.set(
        month,
        (monthTargets.get(month) ?? 0) +
          Number(row[primaryTargetColumn.label] ?? 0),
      );
    });
    ...
    [...monthTargets.entries()].forEach(([month, target], index) => {
      ...
      monthly.addRow([
        month,
        {
          formula: `SUMIF('Production Plan'!$${monthLetter}$2:$${monthLetter}$${lastPlanRow},A${row},'Production Plan'!$${targetOutputLetter}$2:$${targetOutputLetter}$${lastPlanRow})`,
          result: Number(target.toFixed(2)),
        },
        {
          formula: `SUMIF('Production Plan'!$${monthLetter}$2:$${monthLetter}$${lastPlanRow},A${row},'Production Plan'!$${actualOutputLetter}$2:$${actualOutputLetter}$${lastPlanRow})`,
          result: 0,
        },
        {
          formula: `IF(COUNT('Production Plan'!$${actualOutputLetter}$2:$${actualOutputLetter}$${lastPlanRow})=0,"",C${row}-B${row})`,
          result: "",
        },
    ...
    const phaseSheet = workbook.addWorksheet("Phases & Risks", {
      views: [{ showGridLines: false }],
    });
    ...
    risks.forEach((risk) =>
      phaseSheet.addRow([risk.risk, risk.impact, risk.mitigation]),
    );
    if (risks.length === 0)
      phaseSheet.addRow([
        "No material risk proposed",
        "",
    ...
    plan.project.assumptions.forEach((assumption, index) =>
      assumptions.addRow([index + 1, assumption]),
    );
